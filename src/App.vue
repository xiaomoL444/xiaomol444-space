...
const { refresh: refreshGlass, reset: resetGlass } = useGlassGlow(home)
...
watch(() => route.meta.title, (title) => {
  document.title = `xiaomol444 · ${typeof title === 'string' ? title : '个人主页'}`
}, { immediate: true })
...
async function finishPageTransition() {
  ...
  await nextTick()
  refreshGlass()
  pageContent.value?.querySelector<HTMLElement>('[data-page-heading]')?.focus({ preventScroll: true })
}

async function cancelPageTransition() {
  pageTransitioning.value = false
  await nextTick()
  refreshGlass()
}
...
<template>
  ...
  <div ref="home" class="home" :class="{ 'home--waiting': !homeRevealed, 'home--intro-active': introVisible }" :inert="!homeInteractive" :aria-hidden="!homeInteractive || undefined">
    ...
    <div class="page-edge page-edge--top" aria-hidden="true">
      <span class="page-edge-lace" :style="{ '--lace-image': `url(${decorationUrl}svg/video-lace-top.svg)` }" />
    </div>
    <div class="page-edge page-edge--bottom" aria-hidden="true">
      <span class="page-edge-lace" :style="{ '--lace-image': `url(${decorationUrl}svg/video-lace-top.svg)` }" />
    </div>
    ...
    <main ref="pageContent" class="route-content" :inert="pageTransitioning" :aria-busy="pageTransitioning || undefined">
      <RouterView v-slot="{ Component, route: viewRoute }">
        <Transition
          name="route-fade"
          mode="out-in"
          @before-leave="beginPageTransition"
          @after-leave="onPageLeft"
          @after-enter="finishPageTransition"
          @enter-cancelled="cancelPageTransition"
          @leave-cancelled="cancelPageTransition"
        >
